...
class Model(nn.Module):
    ...
    def fault_prediction(self, x_enc, x_mark_enc):
        ...
        adj_matrix_1 = np.eye(n_vars * enc_out.shape[-2], k = 1)
        for i in range(0, n_vars):
            adj_matrix_1[i*enc_out.shape[-2], i*enc_out.shape[-2]+1] = 0
            adj_matrix_1[i*enc_out.shape[-2]:(i+1)*enc_out.shape[-2], i*enc_out.shape[-2]:(i+1)*enc_out.shape[-2]] = np.eye(enc_out.shape[-2])
        ...
        gat_out_1 = torch.reshape(
            gat_out_1, (enc_out.shape[0], enc_out.shape[1], enc_out.shape[2]))


        enc_out = enc_out + gat_out_1
