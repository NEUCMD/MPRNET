...
class Model(nn.Module):
    ...
    def __init__(self, configs, patch_len=4, stride=2):
        ...
        if self.task_name == 'fault_prediction':
            ...
            self.projection = nn.Linear(
               self.enc_in*self.seq_len*configs.d_model, len(utils.global_var.get_value('class_names')))
            ...
            self.flatten = nn.Flatten(start_dim=-2)
            self.projection = nn.Linear(configs.seq_len * configs.d_model * configs.enc_in, configs.seq_len)
            self.dropout = nn.Dropout(configs.dropout)
        elif self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            self.flatten = nn.Flatten(start_dim=-2)
            self.linear = nn.Linear(configs.seq_len * configs.d_model, configs.seq_len)
            self.dropout = nn.Dropout(configs.dropout)
    ...
    def forecast(self, x_enc, x_mark_enc,  
        ...
        enc_out = torch.reshape(
            hidden_states, (-1, n_vars, hidden_states.shape[-2], hidden_states.shape[-1]))
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, attributes=None, mask=None):
        ...
        if self.task_name == 'rul':
            dec_out = self.rul(x_enc, x_mark_enc)
            return dec_out  # [B, N]
