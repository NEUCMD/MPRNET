...
class MultiScaleRetention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.past_linear_up = nn.Linear(config.d_model, config.d_ff, bias=False)
        self.past_linear_down = nn.Linear(config.d_ff, config.d_model, bias=False)
        self.gat_linear = nn.Linear(int(config.n_heads*config.d_model), config.d_model, bias=False)
...
class Model(nn.Module):
    ...
    def __init__(self, configs, patch_len=8):
        ...
        if self.task_name == 'fault_prediction':
            ...
            self.inverted_projection = nn.Linear(self.seq_len*configs.d_model, configs.d_model)
            self.projection = nn.Linear(
               self.enc_in*configs.d_model, len(utils.global_var.get_value('class_names')))
    ...
    def fault_prediction(self, x_enc, x_mark_enc,  
        ...
        hidden_states = torch.reshape(hidden_states, (batchsize, self.enc_in, -1))
        hidden_states = self.inverted_projection(hidden_states) 

        retention_mask = torch.ones((hidden_states.shape[0], self.enc_in),
                                                        dtype=torch.bool,
                                                        device=x_enc.device)
