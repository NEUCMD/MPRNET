...
class Model(nn.Module):
    def __init__(
        ...
        H = configs.pred_len
        r = 6
        r_hat = configs.d_model
        ##TODO这个感觉不对应该要改
        p = configs.d_model
        hidden_dim = configs.d_ff
        encoder_layer_num = configs.e_layers
        decoder_layer_num = configs.d_layers


        featureProjectionHidden = configs.d_model
        temporalDecoderHidden = configs.d_model
        ...
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            ...
            self.encoder = Encoder(encoder_layer_num, hidden_dim, r, r_hat, L, H, featureProjectionHidden)
            self.decoder = Decoder(decoder_layer_num, hidden_dim, r_hat, H, p, temporalDecoderHidden)
            self.residual = nn.Linear(L, configs.num_class)
            self.dropout = nn.Dropout(configs.dropout)
            self.projection = nn.Linear(L, configs.num_class)
        elif self.task_name == 'fault_prediction':
            self.encoder = Encoder(encoder_layer_num, hidden_dim, r, r_hat, L, H, featureProjectionHidden)
            self.decoder = Decoder(decoder_layer_num, hidden_dim, r_hat, L, p, temporalDecoderHidden)
            self.residual = nn.Linear(L,  L)
            self.dropout = nn.Dropout(configs.dropout)
            self.projection = nn.Linear(L*self.enc_in, len(utils.global_var.get_value('class_names')))

    def forecast(self, x, covariates, x_dec, x_mark_dec):
        # x: [B,L,N], covariates: [B,L+H,N,r], attributes: [B,N,1]
        batch_size = x.size(0)
        covariates = covariates.unsqueeze(-2)
        covariates = covariates.repeat(1, 1, self.enc_in, 1)
        attributes = attributes.unsqueeze(1)
        attributes = attributes.unsqueeze(1)
        attributes = attributes.repeat(1, self.enc_in, 1)
    ...
    def fault_prediction(self, x, covariates, attributes):
    # x: [B,L,N], covariates: [B,L+H,N,r], attributes: [B,N,1]
        batch_size = x.size(0)
        ...
        e, covariates_future = self.encoder(x, covariates, attributes)
        # Decoder
        out = self.decoder(e, covariates_future)  # out: [B*N,p,1]

        # Global Residual
        prediction = out.squeeze(-1) + self.residual(x)  # prediction: [B*N,L]       

        # Reshape
        prediction = rearrange(prediction, '(b n) h -> b (h n)', b=batch_size)  # [B*N,L] -> [B,L,N]

        prediction = self.dropout(prediction)
        prediction = self.projection(prediction)  # [batch_size, num_classes]
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, attributes, mask=None):
        ...
        if self.task_name == 'fault_prediction':
            dec_out = self.fault_prediction(x_enc, x_mark_enc, attributes)
            return dec_out  # [B, N]
        return None
